...
INPUT_DIR="./public/assets/images/teacher"
# Папка для сохранения конвертированных изображений
OUTPUT_DIR="./public/assets/images/teacher"
...
find "$INPUT_DIR" -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" \) | while read -r img; do
    ...
    for size in 500; do
        ...
        magick "$img" -resize "${size}x" -quality 80 "$output_subdir/${filename_no_ext}-${size}.${filename##*.}"
    done

    # Конвертация в WebP
    for size in 500; do
        magick "$img" -resize "${size}x" -quality 80 "$output_subdir/${filename_no_ext}-${size}.webp"
    done
    
    # Конвертация в AVIF
    for size in 500; do
        magick "$img" -resize "${size}x" -quality 80 "$output_subdir/${filename_no_ext}-${size}.avif"
    done
done
